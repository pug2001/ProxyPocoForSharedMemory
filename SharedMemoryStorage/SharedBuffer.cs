...
using SharedMemoryStorage.SharedMemory;
...
namespace SharedMemoryStorage
{
    public abstract unsafe class SharedBuffer : IDisposable
    {
        ...
        public virtual long SharedMemorySize
        {
            get
            {
                return HeaderOffset + Marshal.SizeOf(typeof(SharedBufferHeader)) + BufferSize;
            }
        }
        ...
        protected virtual long BufferOffset
        {
            get
            {
                return HeaderOffset + Marshal.SizeOf(typeof(SharedBufferHeader))+Header->ObjectTable->ObjectTableSize();
            }
        }
        ...
        protected bool Open()
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    using (var headerView = Mmf.CreateViewAccessor(0, HeaderOffset + Marshal.SizeOf(typeof(SharedBufferHeader)), MemoryMappedFileAccess.Read))
                    {
                        byte* headerPtr = null;
                        headerView.SafeMemoryMappedViewHandle.AcquirePointer(ref headerPtr);
                        var header = (SharedBufferHeader*)(headerPtr + HeaderOffset);
                        BufferSize = header->SharedMemorySize - Marshal.SizeOf(typeof(SharedBufferHeader));
                    ...
                    View.SafeMemoryMappedViewHandle.AcquirePointer(ref ViewPtr);
                    Header = (SharedBufferHeader*)(ViewPtr + HeaderOffset);
                    BufferStartPtr = ViewPtr + HeaderOffset + Marshal.SizeOf(typeof(SharedBufferHeader));
                }
            }
            catch
            {
